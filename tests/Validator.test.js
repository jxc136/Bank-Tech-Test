...
describe("Validator", () => {
  test("valid deposit object", () => {
    ...
    const balance = 0;
    const deposit = { date: "11/12/2021", credit: 0, debit: 10.0 };
  ...
  test("no date", () => {
    const validator = new Validator();
    const balance = 0;
    const deposit = { credit: 0, debit: 10.0 };
    expect(validator.validate(balance, deposit)).toEqual(false);
  });
  ...
  test("validates deposit below 1000", () => {
    const validator = new Validator();
    const balance = 0;
    const deposit = { date: "05/10/2018", credit: 0, debit: 1000.0 };
    expect(validator.validate(balance, deposit)).toEqual(true);
  });
  ...
  test("withdraw below 1000", () => {
    const validator = new Validator();
    const balance = 1000;
    const withdraw = { date: "05/10/2018", credit: 999.0, debit: 0 };
